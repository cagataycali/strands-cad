...
- `orca-slicer` **or** `bambu-studio` for `slice_bambu` — OrcaSlicer is recommended (works on Apple Silicon / ARM64); either is auto-detected, or point `STRANDS_CAD_SLICER` at the binary. A containerized OrcaSlicer is also available (`STRANDS_CAD_SLICER_DOCKER`).
